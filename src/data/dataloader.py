...
from src.helpers.constants import ALL_WINDOW_SIZES, FAR_FIELD_RADIUS
from src.data_augmentation import RandomAudioPerturbation
...
class SyntheticDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if np.random.uniform() < self.negatives:
            target_angle,spk_id = self.get_negative_region(metadata, candidate_angles)
        else:
            target_angle,spk_id = get_positive_region(metadata, candidate_angles)

        all_sources, target_voice_data = self.get_mixture_and_gt(
            metadata, curr_dir, target_angle, curr_window_size)
        spk_audio_file = glob("/home/bjwoo/PycharmProjects/data/sitec/**/"+spk_id+"/*.wav",recursive=True)
        rand_spk_num = random.randint(1,len(spk_audio_file)-1)
        ref_data, _ = librosa.core.load(spk_audio_file[rand_spk_num], sr=self.sr, mono=True)
        ref_data, _ = librosa.effects.trim(ref_data, top_db=30)

        if len(ref_data) < target_voice_data[0].size(1):
            len_pad = target_voice_data[0].size(1)-len(ref_data)
            ref_data = np.pad(ref_data,(0,len_pad),'constant',constant_values=(0,0))
        else:
            ref_data = ref_data[0:target_voice_data[0].size(1)]
        ...
        return (mixed_data, target_voice_data, window_idx_one_hot,ref_data)
    ...
    def get_negative_region(self, metadata, candidate_angles):
        ...
        spk_id = str(metadata[random_key]["speaker_id"])
    ...
    def get_mixture_and_gt(self, metadata, curr_dir, target_angle,
        ...
        for key in metadata.keys():
            ...
            for _, gt_audio_file in enumerate(gt_audio_files):
                gt_waveform, _ = librosa.core.load(gt_audio_file, sr=self.sr,
                                                   mono=True)
